...
type externalStartupEngine struct {
	path    string
	version string
	logger  Logger
...
func NewManagedStartupEngine(path, version string, logger Logger) Engine {
	return &externalStartupEngine{
		path:    path,
		version: version,
		logger:  logger,
...
func (e *externalStartupEngine) Configure(cfg Config) error {
	envelope := configEnvelope{}
	_, err := e.call(context.Background(), methodConfigure, envelope)
	return err
}

func (e *externalStartupEngine) HandleCommand(ctx context.Context, cmd protocol.Command) protocol.CommandResult {
...
func (e *externalStartupEngine) Shutdown() {
	e.mu.Lock()
	defer e.mu.Unlock()
...
func (e *externalStartupEngine) call(ctx context.Context, method string, params any) (*ipcResponse, error) {
	e.mu.Lock()
	if e.cmd == nil {
...
func (e *externalStartupEngine) startLocked() error {
...
func (e *externalStartupEngine) logStderr(r io.ReadCloser) {
	defer r.Close()
	buf := make([]byte, 1024)
	for {
		n, err := r.Read(buf)
		if n > 0 && e.logger != nil {
			e.logger.Printf("external-startup: %s", string(buf[:n]))
		}
		if err != nil {
			break
		}
...
func (e *externalStartupEngine) readLoop() {
	for {
