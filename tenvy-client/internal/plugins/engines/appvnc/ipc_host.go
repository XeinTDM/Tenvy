...
const (
	...
	methodHandleInputBurst = "handle_input_burst"
...
type configEnvelope struct {
	...
	WorkspaceRoot  string        `msgpack:"workspace_root"`
...
func ServeEngineIPC(ctx context.Context, engine Engine, reader io.Reader, writer io.Writer, logger Logger, clients HTTPClientFactory) error {
	if engine == nil {
		return errors.New("app-vnc engine not provided")
	}
	...
	handle := func(req ipcRequest) ipcResponse {
		...
		case methodHandleCommand:
			...
			respond.Result = data
		case methodHandleInputBurst:
			var payload protocol.AppVncInputBurst
			if err := msgpack.Unmarshal(req.Params, &payload); err != nil {
				respond.Error = &ipcError{Message: fmt.Sprintf("decode input burst payload: %v", err)}
				return respond
			}
			if err := engine.HandleInputBurst(ctx, payload); err != nil {
				respond.Error = &ipcError{Message: err.Error()}
			}
		case methodShutdown:
			engine.Shutdown(ctx)
