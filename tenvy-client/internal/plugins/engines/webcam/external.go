...
type externalWebcamEngine struct {
	path    string
	version string
	logger  Logger
...
func NewManagedWebcamEngine(path, version string, logger Logger) Engine {
	return &externalWebcamEngine{
		path:    path,
		version: version,
		logger:  logger,
...
func (e *externalWebcamEngine) Configure(cfg Config) error {
...
func (e *externalWebcamEngine) HandleCommand(ctx context.Context, cmd protocol.Command) protocol.CommandResult {
...
func (e *externalWebcamEngine) Shutdown() {
	e.mu.Lock()
	defer e.mu.Unlock()
...
func (e *externalWebcamEngine) call(ctx context.Context, method string, params any) (*ipcResponse, error) {
	e.mu.Lock()
	if e.cmd == nil {
...
func (e *externalWebcamEngine) startLocked() error {
...
func (e *externalWebcamEngine) logStderr(r io.ReadCloser) {
	defer r.Close()
	buf := make([]byte, 1024)
	for {
		n, err := r.Read(buf)
		if n > 0 && e.logger != nil {
			e.logger.Printf("external-webcam: %s", string(buf[:n]))
		}
		if err != nil {
			break
		}
...
func (e *externalWebcamEngine) readLoop() {
	for {
