...
type externalAudioEngine struct {
	path    string
	version string
	logger  Logger
...
func NewManagedAudioEngine(path, version string, logger Logger) Engine {
	return &externalAudioEngine{
		path:    path,
		version: version,
		logger:  logger,
...
func (e *externalAudioEngine) Configure(cfg Config) error {
...
func (e *externalAudioEngine) HandleCommand(ctx context.Context, cmd protocol.Command) protocol.CommandResult {
...
func (e *externalAudioEngine) Shutdown() {
	e.mu.Lock()
	defer e.mu.Unlock()
...
func (e *externalAudioEngine) call(ctx context.Context, method string, params any) (*ipcResponse, error) {
	e.mu.Lock()
	if e.cmd == nil {
...
func (e *externalAudioEngine) startLocked() error {
...
func (e *externalAudioEngine) logStderr(r io.ReadCloser) {
	defer r.Close()
	buf := make([]byte, 1024)
	for {
		n, err := r.Read(buf)
		if n > 0 && e.logger != nil {
			e.logger.Printf("external-audio: %s", string(buf[:n]))
		}
		if err != nil {
			break
		}
...
func (e *externalAudioEngine) readLoop() {
	for {
